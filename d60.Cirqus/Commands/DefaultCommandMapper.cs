...
namespace d60.Cirqus.Commands
{
    ...
    public class DefaultCommandMapper : ICommandMapper
    {
        public Action<ICommandContext, Command> GetCommandAction(Command command)
        {
            ...
            throw new ArgumentException(string.Format(@"Could not find a command mapping for the command {0} - please derive your command off of ExecutableCommand or the generic Command<TAggregateRoot>, or supply an action mapping when configuring the command processor", command));
        }
    }
}